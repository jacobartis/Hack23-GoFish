...
@commands.has_role("fishe")
@bot.command(name="video",help = "getsVideo")
async def video(ctx):
    x = Servo.video()
    print("video recorded")
    await ctx.channel.send(file=discord.File('./flask_server/services/fishgo_service/output.avi'))
    if x.status_code == 200:
        await ctx.channel.send("video created")
    else:
        await ctx.channel.send("video failed")
